...
**D2 — Per-agent skill composition (the "lightening").** The resolver yields the Spotlight skill set for the `pi` runtime; Flue composes **subsets per sub-agent** — investigator gets research/OSINT skills, fact-checker gets verification skills, orchestrator gets planning. No agent holds all ~10. This lives in the harness adapter, **never in the skills**. *(U2 shipped the map, 2026-07-06: `harness/composition.json` — orchestrator {ingest, monitoring, report-drafting, review, provenance-signing, integrations}, investigator {osint, investigate, follow-the-money, integrations, content-access, web-archiving, epistemic-grounding, shell-safety, acquisition-graduation, social-media-intelligence}, fact-checker {osint, content-access, web-archiving, epistemic-grounding, shell-safety, acquisition-graduation, social-media-intelligence}; grounded in `agents/*.md` invoke-skill() + orchestrator §3.5. `harness/validate_composition.py` proves union = 15/15 and the **worst-agent manifest floor = ~9.3K tok** (role + bundle descriptions; bodies on-invoke) — meets ≤20K. Consumed by U7.)*
