...
MENU = {
    "items": [
        {
            "id": "t_egypt",
            "slug": "social-media-reactions-to-egypt-vs-fifa-complaint",
            "title": "Social Media Reactions to Egypt vs FIFA Complaint",
            "description": "Egypt complains to FIFA against Argentina",
        ...
        {
            "id": "t_gadkari",
            "slug": "social-media-coverage-of-nitin-gadkari-ethanol-scam",
            "title": "Social Media Coverage of Nitin Gadkari Ethanol Scam",
            "description": "nitin gadkari ethanol scam",
            "platforms": ["twitter"],
        ...
        {
            "id": "t_mandir",
            "slug": "ram-mandir-theft-claims-on-social-media",
            "title": "Ram Mandir Theft Claims on Social Media",
            "description": "theft claims about the ram mandir temple donations",
            "platforms": ["youtube"],
...
def main() -> int:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        result = run(menu, "--query", "egypt fifa complaint")
        assert result["matches"], "expected a match for a direct query"
        top = result["matches"][0]
        assert top["id"] == "t_egypt", top
        assert top["score"] == 1.0, top
        assert sorted(top["matched_terms"]) == ["complaint", "egypt", "fifa"], top
        assert top["entity_count"] == 18, top
        assert top["starred"] is True, top

        # 2. Partial overlap still surfaces the right case study.
        result = run(menu, "--query", "temple theft rumours")
        assert [m["id"] for m in result["matches"]] == ["t_mandir"], result["matches"]
        ...
        assert {"t_egypt", "t_gadkari", "t_mandir"} <= other_ids, other_ids

        # 4. Prefix match earns partial credit only.
        result = run(menu, "--query", "egyptian", "--threshold", "0.1")
        assert result["matches"], "prefix match should clear a low threshold"
        assert result["matches"][0]["id"] == "t_egypt"
        ...
        result = run(menu, "--query", "the social media story of egypt")
        assert [m["id"] for m in result["matches"]] == ["t_egypt"], result["matches"]

        # 6. Malformed items were tolerated throughout (implicit in the runs
        #    above) and never appear in the output.
        result = run(menu, "--query", "egypt")
        ...
        # 7. --query-file equals --query for hostile free text.
        hostile = "what about gadkari's ethanol \"scam\"? 100% $(true)"
        ...
        assert via_file["matches"][0]["id"] == "t_gadkari"
        ...
        result = run(menu, "--query", "social media egypt gadkari mandir", "--top", "1")
