...
|---|---:|---:|---|---|
| PyPI (uv tool) | `crawl4ai` | `0.9.0` | `crwl` | Sovereign scrape default (`fetch`). Installed via `uv tool install crawl4ai` + `crawl4ai-setup` (Playwright browser runtime). Reviewed 2026-07-08. |
...
| Ecosystem | Package | Version | Binary | Install Policy |
|---|---:|---:|---|---|
| npm | `firecrawl-cli` | `1.3.1` | `firecrawl` | **Optional escape hatch** (KTD4/KTD6): scrape fallback on a hard bot-block + optional search-union. Installed only when the Firecrawl fallback is enabled (`FIRECRAWL_API_KEY` present); absent = pure-sovereign. |
