...
def main():
    ...
    def forget_auth_flow(flow_id):
        timer = auth_flow_timers.pop(flow_id, None)
        if timer:
            timer.cancel()
        auth_flows.pop(flow_id, None)

    def cancel_pending_auth_flow(flow_id):
        if flow_id not in auth_flows:
            return
        try:
            engine_bridge.navigator_cancel(flow_id)
        except Exception:
            pass
        finally:
            forget_auth_flow(flow_id)
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            if origin and origin != "http://" + self.headers.get("Host", ""):
                self._send(403, "cross-origin request rejected", "text/plain")
                return
            ...
            elif parsed.path == "/engine-auth/navigator/status":
                try:
                    self._send(200, json.dumps(engine_bridge.navigator_status()))
                except Exception as error:
                    self._send(400, json.dumps({"error": str(error)}))
            elif parsed.path == "/engine-auth/navigator/poll":
                flow_id = query.get("flow_id", [""])[0]
                email = auth_flows.get(flow_id)
                if not email:
                    self._send(410, json.dumps({"status": "expired"}))
                    return
                try:
                    response = engine_bridge.navigator_poll(flow_id, email)
        ...
        def do_POST(self):
            if self.path == "/submit":
                ...
                return
            if self.path not in ("/pick-folder", "/engine-submit", "/engine-auth/navigator/start", "/engine-auth/navigator/cancel", "/engine-auth/navigator/logout"):
                self._send(404, "not found", "text/plain")
                return
            try:
            ...
            if origin and origin != "http://" + self.headers.get("Host", ""):
                ...
                return
            if self.path == "/engine-auth/navigator/start":
                try:
                    email = str(payload.get("email") or "").strip()
                    response = engine_bridge.navigator_start(email)
                    auth_flows[response["flow_id"]] = email
                    lifetime = max(1, int(response.get("expires_in_seconds") or 900) - 5)
                    timer = threading.Timer(lifetime, cancel_pending_auth_flow, args=(response["flow_id"],))
                    timer.daemon = True
                    auth_flow_timers[response["flow_id"]] = timer
                    timer.start()
                    self._send(200, json.dumps(response))
                except Exception as error:
                    self._send(400, json.dumps({"error": str(error)}))
                return
            if self.path == "/engine-auth/navigator/cancel":
                flow_id = str(payload.get("flow_id") or "")
                try:
                    response = engine_bridge.navigator_cancel(flow_id)
                    forget_auth_flow(flow_id)
                    self._send(200, json.dumps(response))
                except Exception as error:
                    self._send(400, json.dumps({"error": str(error)}))
                return
            if self.path == "/engine-auth/navigator/logout":
                try:
                    self._send(200, json.dumps(engine_bridge.navigator_logout()))
                except Exception as error:
                    self._send(400, json.dumps({"error": str(error)}))
                return
    ...
    try:
        ...
        finished = False
        print("\n  Aborted.")
    for flow_id in list(auth_flows):
        cancel_pending_auth_flow(flow_id)
    server.shutdown()
