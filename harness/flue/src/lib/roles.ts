...
- **query-vault(query)** → the OpenKnowledge MCP \`mcp__openknowledge__search\` tool against the configured workspace. Return normalized paths, backend, readiness, and ranked results. If OpenKnowledge reports unready or unavailable, use exact read-only Markdown search and label every result \`markdown_fallback\`; never treat an unready empty result as "not found".
- **vault-write(workspace_path, note_path, content)** → the matching OpenKnowledge MCP write/edit tools. Durable writes still require the approval and journal contract in the active skill; never replace them with a direct shell write.
