...
**Report gate (deterministic).** After Gate 1 approval and BEFORE presenting the final gate, run \`bash\`: \`python3 ${HARNESS_ROOT}/scripts/finalize-report.py <CASE_DIR>\`. This validates the evidence chain, deterministically renders \`findings-report.md\`, \`report.html\`, and \`evidence-map.json\` from the structured data, then validates the outputs. NEVER copy, populate, regex, or hand-edit the HTML template. If it FAILS, fix only the structured input named by the failed stage and re-run. Never describe an artifact as "ready" or "publication-ready" unless the finalizer prints \`report finalizer: PASSED\` — completion is what the finalizer says it is, not what the narrative says.`,
	compaction: COMPACTION,
