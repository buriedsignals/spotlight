...
**Report gate (deterministic).** After drafting the report artifacts and BEFORE presenting the final gate, run \`bash\`: \`python3 ${HARNESS_ROOT}/scripts/validate-report.py <CASE_DIR>\`. If it FAILS, fix the listed issues yourself (populate \`report.html\` with the case content — a template copy is not a report; create or stop referencing missing files; align the report's confidence column with the fact-check statuses) and re-run until it passes, or present the gate with the remaining failures disclosed verbatim. Never describe an artifact as "ready" or "publication-ready" that the validator rejects — completion is what the validator says it is, not what the narrative says.`,
	compaction: COMPACTION,
