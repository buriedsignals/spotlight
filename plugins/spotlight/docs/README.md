...
3. **[integrations.md](integrations.md)** — external OSINT tool integrations (dev-browser, Junkipedia, Arbiter, Noosphere C2PA, OSINT Navigator, Unpaywall), manifest contract, preflight
   - **[arbiter-api.md](arbiter-api.md)** — Spotlight's local reference for the Arbiter partner integration behind Data Navigator's BYO-key source: endpoints, auth, credits, pagination, and lifecycle
