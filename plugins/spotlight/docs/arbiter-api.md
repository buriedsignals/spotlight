...
Arbiter is a social-media case-study platform. A *case study* is a bounded corpus — one search query, a set of platforms, a date window — that has been collected, de-duplicated, indexed and analysed. Spotlight calls this API directly over HTTPS for members who provide their own Arbiter key, for browsing existing studies, creating new ones, and asking the case-study-scoped agent.

Arbiter is a partner integration. This file is Spotlight's local reference for the upstream interface (what the endpoints accept and return), while `integrations/arbiter/integration.md` is the agent-facing native HTTPS contract (how Spotlight drives the flow).
...
| Configuration | Set the member-owned `ARBITER_API_KEY` in local approved secret storage; optionally set `ARBITER_API_BASE` for the matching deployment. |
...
**Getting a key:** each member registers through the [Indicator partner signup link](https://arbiter.simppl.org/auth/register?eventSignup=5cce20c609334e538f07127322361862e3136e3d-324a-4c60-894a-5f42d2d57f8a), signs in, opens the user profile page → **API Keys** tab, creates a key (optionally naming it, 1–60 characters), and copies the plaintext immediately — it is shown exactly once. Configure it locally as `ARBITER_API_KEY`; do not paste it into Spotlight, a case file, or a shared service. There is no shared Buried Signals or Spotlight key.
