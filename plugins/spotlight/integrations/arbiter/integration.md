...
The call **blocks until the agent finishes** — tens of seconds typically, up to ~10 minutes — so always pass a long `--max-time`. The response carries `answer` (markdown), `run_id`, and `meta.credits_charged`. Ask one question at a time per study; a concurrent question returns 429 with `Retry-After: 30`. Reproduce the `answer` markdown in full and unmodified, tables included.
