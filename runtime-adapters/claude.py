---
name: claude-adapter
description: Claude Code runtime adapter for the Spotlight OSINT system
version: "1.0"
runtime: claude
---
...
Implements the tool verb registry for Claude Code environments.

## Implementation Notes

This adapter wraps Claude Code's native tools to satisfy the verb contract defined in `AGENTS.md`.

## Usage

```python
from runtime_adapters.claude import ClaudeAdapter
...
Scrape URL content using Claude's `WebFetch` tool, save to file, return metadata.

#### search(query: str, output_path: str, limit: int = 10) -> dict
Execute web search using Claude's `WebSearch` tool, save results to file.

#### read_file(path: str) -> str
Read file contents using Claude's `Read` tool.

#### write_file(path: str, content: str) -> dict
Write content to file using Claude's `Write` tool.

#### edit_file(path: str, old: str, new: str) -> dict
Edit file using Claude's `Edit` tool with targeted string replacement.

#### list_files(pattern: str) -> list[str]
Use glob pattern matching to find files.

#### grep_files(pattern: str, path: str) -> list[dict]
Search file contents using regex via shell grep.
...
Launch Claude session as sub-agent, return handle.

#### wait_agent(handle: str) -> dict
Block until agent completes using `wait_for_session`.
...
Search knowledge vault using content search.
...
class ClaudeAdapter:
    """Claude Code runtime adapter for Spotlight OSINT system."""
    ...
    def fetch(self, url: str, output_path: str) -> dict:
        ...
        # Claude Code WebFetch implementation
        # This would integrate with Claude's actual web fetch tool
        raise NotImplementedError("Claude fetch not yet implemented")
    ...
    def search(self, query: str, output_path: str, limit: int = 10) -> dict:
        ...
        # Claude Code WebSearch implementation
        raise NotImplementedError("Claude search not yet implemented")
    ...
    def wait_agent(self, handle: str) -> dict:
        ...
        # In a real implementation, this would block until the Claude session completes
