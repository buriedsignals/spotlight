...
install_python_reviewed_deps() {
  ...
  if [ "$DRY_RUN" = "1" ]; then
    ...
    printf 'DRY-RUN: %s/bin/python -m pip install --quiet jsonschema==%s requests==%s crawl4ai==%s navigator-cli==%s\n' \
      "$SPOTLIGHT_PYTHON_ENV" "$JSONSCHEMA_VERSION" "$REQUESTS_VERSION" "$CRAWL4AI_VERSION" "$NAVIGATOR_CLI_VERSION"
  ...
  "$SPOTLIGHT_PYTHON_ENV/bin/python" - "$JSONSCHEMA_VERSION" "$REQUESTS_VERSION" "$CRAWL4AI_VERSION" "$NAVIGATOR_CLI_VERSION" <<'PY'
...
if [ "$DRY_RUN" = "1" ]; then
  printf 'DRY-RUN: write .env with OSINT_NAV_API_KEY, optional FIRECRAWL_API_KEY, %s + integration keys\n' "${SPOTLIGHT_CLOUD_KEY_VAR:-<none>}"
else
...
check_cmd navigator "Navigator CLI"
